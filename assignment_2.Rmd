...
```{r}
fit_2 <- glm(Churn ~tenure+Contract+PaperlessBilling+TotalCharges, 
             family=binomial(link='logit'), 
             data= churn_data) 
summary(fit_2) 
```
...
```{r}
fit_3 <- glm(Churn ~tenure+Contract+PaperlessBilling*TotalCharges, 
             family=binomial(link='logit'), 
             data= churn_data) 
summary(fit_3) 
```
...
The additional interaction term decrease the AIC from 6254.8 to 6242

* 7 - Fit a lasso logistic regression model to the data. (1 Point)
```{r}
x_vars <- model.matrix(Churn ~., 
                       churn_data)[,-1]
x_data <- as.data.frame(scale(x_vars[,-31]))
x_vars <- as.matrix(x_data)
...
best.lasso <- glmnet(x = x_vars,
          ...
          lambda = best_lam)
coef(best.lasso)
lasso_coef <- as.data.frame(as.matrix(coef(best.lasso)))
library(dplyr)
row.names(lasso_coef %>% filter(s0 > 0.00000001))
```
part of them are positive and part of them are negative, this imply their relationship with the model.
