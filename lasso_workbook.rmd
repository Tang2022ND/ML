...
```
```{r}
t_bins <- bin(house_price$House.Price, nbins = 6, method = "length") 
plot_dat <- cbind.data.frame(t_bins, house_price)
gg_miss_fct(x = plot_dat, fct = t_bins) +
  labs(x = "house_price")
```
...
```{r}
imputed_values <- mice(data = house_price[,2:dim(house_price)[2]],
                       m = 1,
                       maxit = 40,
                       method = "cart",
                       print = F)
house_price[,2:dim(house_price)[2]] <- complete(imputed_values, 1)
summary(house_price)
```

-   Run a linear regression with housing price as the response variable (Leave out country).
```{r}
lm_full <- lm(House.Price~., data = house_price[,2:dim(house_price)[2]])
summary(lm_full)
```

-   Which variables are significant in the linear regression?
```{r}
lm_bwd <- step(lm_full, k = log(dim(house_price)[1]), direction = "backward")
var_list <- data.frame(summary(lm_full)$coefficient)
row.names(var_list[var_list$Pr...t.. <= 0.1,])
